<template>
  <div class="app-container" style="background-color: #eee;">
    <el-row :gutter="20">
      <el-col :span="6">
        <el-card class="box-card" style="height: 85vh;">
          <div slot="header" class="clearfix">
            <span>组件列表</span>
            <p class=" mt-1" style="font-size: 10px;color:#bbb">
              点击组件添加至列表
            </p>
          </div>
          <el-row :gutter="20">
            <el-col :span="12" style="margin-bottom: 10px;" v-for="(item, index) in components" :key="index">
              <el-card :body-style="{ padding: '15px 5px', cursor: 'pointer' }" style="overflow: inherit;"
      ...
      <el-col :span="8">
        <el-card style="min-height: 85vh; overflow: inherit;" :body-style="{ padding: '10px 8px' }">
          <img src="./status.png" style="width: 100%; margin-bottom: 10px;" />
          <div style="margin: 0 -10px">
            <div class="checked-box " @click="handlechoose(item)" :class="item.checked ? 'checked-box-active' : ''"
              ...
              <div v-if="item.checked" class="check-box-btn">
                ...
                <i class="el-icon-bottom" @click.stop="moveDown(index)"
              ...
              <template v-if="item.type == 'list'">
                <list :showMore="item.showMore" :title="item.title" :list="item.data" :listType="item.listType"></list>
                <!-- 列表组件位置 -->
              </template>
      ...
      <div v-if=" CurrentItem.type !=='coupon'&&CurrentItem.type " style=" position: fixed;top: 130px;right: 40px" class="bg-white">
        <el-card style="width: 420px;min-height: 85vh;position: relative;">
