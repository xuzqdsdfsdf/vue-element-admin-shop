    ...
    <el-table
      ...
      <el-table-column label="拼团课程" width="250" align=""  >
        <template slot-scope="{row}">
          <div class="d-flex " style="flex:1;align-items:center;jusity-content:flex-start">
            <img :src="row.value.cover" style="width:80px;height:50px" />
            <div class="d-flex flex-column " style="margin-left:10px;">
              <span class="text-over" style="color:#4091F7;flex:1">{{row.value.title}}</span>
              <div>原始价格：<span style="color:red">￥{{row.price}}</span></div>
              <div>拼团价格：<span style="color:red">￥{{row.value.price}}</span></div>
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="拼团时限" align="center" prop="expire_time" />
